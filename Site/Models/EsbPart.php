...
class EsbPart extends Model
{
    ...
    protected $table = 'esb_repair_parts';
     ...
     */
    public function esbRepair()
    {
        return $this->belongsTo(EsbRepair::class);
    }
    ...
    public function getTotalEuroAttribute()
    {
        ...
        $cacheKey = 'currency_978_' . $this->esbRepair->getAttribute('date_repair');

        return cache()->remember($cacheKey, config('site::cache.ttl'), function () {
            $rates = CurrencyArchive::query()
                ->select(array('rates'))
                ->where('currency_id', 978)
                ->where('date', $this->esbRepair->getAttribute('date_repair'))
    ...
    public function getRatesAttribute()
    {
        ...
        return Site::currencyRates($this->product->repairPrice->currency, $this->esbRepair->user->currency, $this->esbRepair->getAttribute('date_repair'));
