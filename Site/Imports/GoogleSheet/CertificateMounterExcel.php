...
class CertificateMounterExcel
{
	...
	public function parse(string $engineer_id)
	{
		...
		if(($spreadsheet_id = env('SPREADSHEET_MOUNTER_ID', false)) === false) {
			throw new CertificateException(trans('site::certificate.error.spreadsheet_id_not_found'));
		}
		if(($spreadsheet_range = env('SPREADSHEET_MOUNTER_RANGE', false)) === false) {
		...
		$data = [
			'id' => config('site.certificate_mounter_first_letter', 'MR').Carbon::createFromFormat('d.m.Y H:i:s', $certificate[0])->format('ymd').'0'.auth()->user()->getKey().'0'.$engineer_id,
