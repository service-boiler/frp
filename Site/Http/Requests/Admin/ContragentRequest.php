...
//                        'regex:/^$|^\d{9}$/',
    ...
    {
       return ['unique' => 'УЖЕ ЕСТЬ НА САЙТЕ :attribute'];
        //return ['unique' => ':attribute уже есть на сайте <a target="_blank" href="' .route('admin.users.show', Contragent::where('inn',$this->input('contragent.inn'))->first()->user_id) .'">Открыть пользователя</a>'];
    }
    ...
    public function attributes()
    {
        return [
            ...
            //'contragent.inn'             => 'контрагент с таким ИНН <a target="_blank" href="' .route('admin.users.show', Contragent::where('inn',$this->input('contragent.inn'))->first()->user_id) .'">Открыть пользователя</a>',
