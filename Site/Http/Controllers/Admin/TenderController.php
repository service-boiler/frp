...
use ServiceBoiler\Prf\Site\Filters\Tender\TenderManagerFilter;
...
class TenderController extends Controller
{
    ...
    public function __construct(CountryRepository $countries, RegionRepository $regions, TenderRepository $tenders, FileTypeRepository $types, FileRepository $files, MessageRepository $messages )
    {
    ...
    {  
        ...
        $this->tenders->pushTrackFilter(TenderManagerFilter::class);
        $this->tenders->pushTrackFilter(TenderDistributorFilter::class);
        ...
        return view('site::admin.tender.index', [
    ...
    public function show(Tender $tender)
    {
        ...
        $chief = $tender->user->getChiefAttribute();
        //$file_name=(new TenderResultPdf())->setModel($tender)->store('tenders');
                 /*$data[] = [
                    'file'    => $file_name,
                    'options' => [
                        'as'   => 'tender_.pdf',
        ...
        return view('site::admin.tender.show', compact('tender', 'customers','files', 'statuses', 'file_types','commentBox'));
    }
    ...
    public function create(TenderRequest $request)
    {
        ...
        return view('site::admin.tender.create', compact('countries','regions','tenderProducts','tenderCategories','files','products','file_types','planned_purchase_years','rates'));
    }
      ...
      //  $this->authorize('create', Tender::class);
       // dd($request->input());
        //$tender=Tender::findOrFail('1');
         ...
         $request->user()->tenders()->save($tender = $this->tenders->create(
         array_merge(['manager_id'=>$request->user()->id,
                      'head_id'=>$request->user()->chief->id,
        ...
        return redirect()->route('admin.tenders.show', $tender)->with('success', trans('site::tender.created'));
    }
    ...
    public function edit(TenderRequest $request, Tender $tender)
    {
        ...
        return view('site::admin.tender.edit', compact('tender','countries','regions','tenderProducts','tenderCategories','files','products','file_types','planned_purchase_years','statuses','rates'));
    }
    ...
    {   
       ...
       if (!is_null($file_types) && is_array($file_types)) {
        ...
        return redirect()->route('admin.tenders.show', $tender)->with('success', trans('site::tender.updated'));
    }

    public function order(Tender $tender)
    {
        
        $order_products=[];
        foreach($tender->tenderProducts as $tenderProduct) {
            $order_products[$tenderProduct->product_id]=['product_id'=>$tenderProduct->product_id,'quantity'=>$tenderProduct->count,'price'=>round($tenderProduct->cost,0),'currency_id'=>$tenderProduct->currency_id];
        }
        ...
        return redirect()->route('admin.tenders.show', $tender)->with('success', 'Заказ для тендера создан');
    ...
    $tender->save();
    return redirect()->route('admin.tenders.show',$tender)->with('success', 'Контрагент обновлен');
    }
    ...
    {   
        ...
        return redirect()->route('admin.tenders.show', $tender)->with('success', 'Статус тендера обновлен');
    }
