        ...
        <div class="card mb-5">
            <div class="card-body">
                ...
                <form id="form" method="POST" action="{{ route('academy-admin.tests.update',$academyTest) }}">
                    ...
                    <div class="row mb-3">        
                            <div class="col-4">
                                @lang('site::academy.test.count_attached')
                            </div>      
                            <div class="col-1">
                                {{$questions->count()}}
                            </div>
                ...
                <hr/>
                <hr/>
                <h5>Добавление вопросов списком</h5>
                <span class="text-danger">Выбранные ниже вопросы будут добавлены без проверки на дубли. Возможно, нужно будет удалить лишние.</span>
                
                @foreach($questionList as $ql)
                    <div class="custom-control custom-checkbox">
                        <input name="questionList[]"
                               value="{{ $ql->id }}"
                               type="checkbox"
                               class="custom-control-input" id="ql-{{ $ql->id }}">
                        <label class="custom-control-label"
                               for="ql-{{ $ql->id }}">{{ $ql->text }}</label>
                    </div>
                @endforeach 
                </form>
