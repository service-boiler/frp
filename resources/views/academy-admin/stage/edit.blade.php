@extends('layouts.app')
@section('title'){{ $stage->name }}@lang('site::messages.title_separator')@endsection
@section('content')
    <div class="container">
        <ol class="breadcrumb">
        ...
        <h1 class="header-title mb-4">@lang('site::messages.edit') @lang('site::academy.stage.add')</h1>
        ...
        <div class="card mb-5">
            <div class="card-body">
                ...
                <form id="form" method="POST" action="{{ route('academy-admin.stages.update',$stage) }}">
                    @csrf
                     @method('PUT')
                    <div class="form-row">
                                <div class="col mb-3">
                                    ...
                                    <input type="text" name="stage[name]"
                                           ...
                                           value="{{ old('stage[name]', $stage->name) }}">
                                    <span class="invalid-feedback">{{ $errors->first('stage.name') }}</span>
                                </div>
                    </div>
                                ...
                                <div class="input-group">
                                    <select class="form-control{{  $errors->has('stage.theme_id') ? ' is-invalid' : '' }}"
                                        ...
                                        @foreach($themes as $theme)
                                            <option @if(old('stage[theme_id]', isset($stage) ? $stage->theme_id : null) == $theme->id) selected @endif
                                                    value="{{ $theme->id }}">
                                                {{ $theme->name }}
                                            </option>
                                        @endforeach
                    ...
                    <div class="form-row">
                                <div class="col mb-3">
                                    ...
                                    <input type="text" name="stage[annotation]"
                                           ...
                                           value="{{ old('stage[annotation]',$stage->annotation) }}">
                                    <span class="invalid-feedback">{{ $errors->first('stage.annotation') }}</span>
                                </div>
                    </div>
                    ...
                    <div class="row mt-3">        
                            ...
                            <label class="control-label" for="test_id">@lang('site::academy.stage.add_test')</label>
                                <div class="input-group">
                                    <select class="form-control{{  $errors->has('stage.test_id') ? ' is-invalid' : '' }}"
                                        ...
                                        @foreach($tests as $test)
                                            <option @if(old('stage[test_id]', !empty($stage->tests->first()) ? $stage->tests->first()->id : null) == $test->id) selected @endif
                                                    value="{{ $test->id }}">
                                                {{ $test->name }}
                                            </option>
                                        @endforeach
                                ...
                                <div class="input-group">
                                    <select class="form-control{{  $errors->has('stage.parent_stage_id') ? ' is-invalid' : '' }}"
                                        ...
                                        @foreach($stages as $parent_stage)
                                            <option @if(old('stage[parent_stage_id]', $stage->parent_stage_id == $parent_stage->id)) selected @endif
                    ...
                    <div class="row mt-4">  
                            <div class="col-5">
                                 <div class="custom-control custom-checkbox align-text-bottom">
                                                <input type="checkbox"
                                                       @if(old('stage[is_required]', isset($stage) ? $stage->is_required : null) == 1) checked @endif
