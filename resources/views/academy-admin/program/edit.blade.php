@extends('layouts.app')
@section('title'){{$program->name}} @lang('site::messages.edit') @lang('site::academy.program.add') @lang('site::messages.title_separator')@endsection
@section('content')
    <div class="container">
        <ol class="breadcrumb">
        ...
        <h1 class="header-title mb-4">@lang('site::messages.edit') @lang('site::academy.program.add')</h1>
        ...
        <div class="card mb-5">
            <div class="card-body">
                ...
                <form id="form" method="POST" action="{{ route('academy-admin.programs.update',$program) }}">
                    @csrf
                     @method('PUT')
                    <div class="form-row">
                                <div class="col mb-3">
                                    ...
                                    <input type="text" name="program[name]"
                                           ...
                                           value="{{ old('program[name]', $program->name) }}">
                                    <span class="invalid-feedback">{{ $errors->first('program.name') }}</span>
                                </div>
                    </div>
                                ...
                                <div class="input-group">
                                    <select class="form-control{{  $errors->has('program.theme_id') ? ' is-invalid' : '' }}"
                                        ...
                                        @foreach($themes as $theme)
                                            <option @if(old('program[theme_id]', isset($program) ? $program->theme_id : null) == $theme->id) selected @endif
                                                    value="{{ $theme->id }}">
                                                {{ $theme->name }}
                                            </option>
                                        @endforeach
                    ...
                    <div class="form-row">
                                <div class="col mb-3">
                                    ...
                                    <input type="text" name="program[annotation]"
                                           ...
                                           value="{{ old('program[annotation]',$program->annotation) }}">
                                    <span class="invalid-feedback">{{ $errors->first('program.annotation') }}</span>
                                </div>
                    </div>       
                    <div class="form-row">
                                <div class="col mb-3">
                                    <label class="control-label" for="name">@lang('site::academy.program.bonuses')</label>
                                    <input type="number" name="program[bonuses]"
                                           id="bonuses"
                                           class="form-control{{ $errors->has('bonuses') ? ' is-invalid' : '' }}"
                                           value="{{ old('program[bonuses]',$program->bonuses) }}">
                    ...
                    <div class="row">        
                            ...
                            <div class="col-7">
                                ...
                                Сортировка доступна на странице просмотра программы после сохранения.
                                </div>
                    ...
                    <div class="row mt-4">  
                            <div class="col-5">
                                 <div class="custom-control custom-checkbox align-text-bottom">
                                                <input type="checkbox"
                                                       @if(old('program[enabled]', isset($program) ? $program->enabled : null) == 1) checked @endif
