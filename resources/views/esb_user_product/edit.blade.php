...
@section('content')
    <div class="container">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">@lang('site::user.esb_user_product.edit')</li>
        </ol>
        ...
        <div class="row justify-content-center mb-5">
            <div class="col">
                <form id="form"
                      method="POST"
                      action="{{ route('esb-user-products.update',$esbUserProduct) }}">
                    @csrf
                    @method('PUT')
                    <div class="card mb-2">
                        <div class="card-body">
                            ...
                            <div class="form-row mb-3">
                                <div class="col-sm-3">
                                    ...
                                    <input type="text"
                                           ...
                                           value="{{ old('serial',$esbUserProduct->serial) }}">
                                ...
                                <div class="col-sm-3">
                                    ...
                                    <div class="input-group date datetimepicker" id="datetimepicker_date_sale"
                                        ...
                                        <input type="text"
                                               ...
                                               value="{{ old('date_sale',$esbUserProduct->date_sale ? $esbUserProduct->date_sale->format('d.m.Y') : null) }}">
                                ...
                                <div class="col-sm-6">
                                    ...
                                    <input type="text"
                                           ...
                                           value="{{ old('sale_comment',$esbUserProduct->sale_comment) }}"
                            ...
                            <div class="form-row mb-3">
                                ...
                                <select class="form-control{{  $errors->has('address_id') ? ' is-invalid' : '' }}"
                                        name="address_id"
                                        id="address_id">
                                    @if($esbUserProduct->user_id != auth()->user()->id)
                                    <option selected value="{{ $esbUserProduct->address->id }}">{{ $esbUserProduct->address->full }} </option>
                                    @endif
                                    @if($esbUserProduct->user_id == auth()->user()->id && ($addresses->count() == 0 || $addresses->count() > 1))
                                        <option value="">@lang('site::messages.select_from_list')</option>
                                    @endif
                                    @foreach($addresses as $address)
                                        <option
                                                @if(old('address_id',$esbUserProduct->address_id) == $address->id) selected
                                                @endif
                                ...
                                @if($esbUserProduct->user_id != auth()->user()->id)
                                    <span class="text-success">Добавить и изменить адреса можно только в профиле пользователя</span>
                                @else    
                            ...
                            <div class="form-row">
                                ...
                                <select class="form-control{{  $errors->has('equipment_id') ? ' is-invalid' : '' }}"
                                    ...
                                    @foreach($equipments as $equipment)
                                        <option
                                                @if(old('equipment_id',$esbUserProduct->product ? $esbUserProduct->product->equipment_id : null) == $equipment->id) selected
                                                @endif
                            ...
                            <div class="form-row mb-3">
                                ...
                                <select class="form-control{{  $errors->has('product_id') ? ' is-invalid' : '' }}"
                                    ...
                                    @foreach($products as $product)
                                        <option
                                                @if(old('product_id',$esbUserProduct->product ? $esbUserProduct->product_id : null) == $product->id) selected
                                                @endif
                            ...
                            <div class="form-row mb-3">
                                ...
                                <textarea
                                       ...
                                       >{{ old('product_no_cat',$esbUserProduct->product_no_cat) }}</textarea>
                            ...
                            <div class="form-row mb-3">
                                <div class="col-sm-6">
                                    <label class="control-label" for="client">@lang('site::user.esb_user_product.device_id')</label>
                                    <input type="text"
                                           id="device_id"
                                           name="device_id"
                                           class="form-control{{ $errors->has('device_id') ? ' is-invalid' : '' }}"
                                           value="{{ old('device_id',$esbUserProduct->device_id) }}"
                                           placeholder="@lang('site::user.esb_user_product.dealer_placeholder')">
                                    <span class="invalid-feedback">{{ $errors->first('device_id') }}</span>
                                </div>
                            </div>
